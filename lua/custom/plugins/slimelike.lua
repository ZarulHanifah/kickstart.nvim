...
local function find_terminal_buf_and_send_chunk(lines, opts)
  opts = opts or {}
  for _, win in ipairs(vim.api.nvim_list_wins()) do
    local buf = vim.api.nvim_win_get_buf(win)
    if vim.bo[buf].buftype == 'terminal' then
      ...
      if job_id then
        local chunk
        if #lines > 1 then
          chunk = string.format("exec('''\n%s\n''')\n", table.concat(lines, '\n'))
        else
          local line = lines[1]:gsub('^%s+', '')
          chunk = line .. '\n'
        end

        vim.api.nvim_chan_send(job_id, chunk)
...
vim.keymap.set('v', '<leader>xx', function()
  ...
  find_terminal_buf_and_send_chunk(lines, { scroll = true })
end, { desc = 'Send visual selection to terminal' })

vim.keymap.set('n', '<leader>xx', function()
  local line = vim.api.nvim_get_current_line()
  find_terminal_buf_and_send_chunk({ line }, { scroll = true })
...
vim.keymap.set('v', '<leader>xj', function()
  ...
  local lines = vim.api.nvim_buf_get_lines(0, start_row - 1, end_row, false)
  find_terminal_buf_and_send_chunk(lines, { jump = true, scroll = true })
end, { desc = 'Send selection and jump to terminal' })

vim.keymap.set('n', '<leader>xj', function()
  local line = vim.api.nvim_get_current_line()
  find_terminal_buf_and_send_chunk({ line }, { jump = true, scroll = true })
